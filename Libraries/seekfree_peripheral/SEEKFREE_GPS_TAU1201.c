...
{
    handle->rxDataSize = gps_tau1201_receivexfer.dataSize;  //还原缓冲区长度
    handle->rxData = gps_tau1201_receivexfer.data;          //还原缓冲区地址
    if('$' == gps_tau1201_rx_buffer || ('$' != gps_tau1201_buffer[0]))//帧头校验
    {
        gps_tau1201_num = 0;
    }
