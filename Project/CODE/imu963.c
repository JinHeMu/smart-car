...
//		source_data.gyro_z = Movingaverage_filter(imu963ra_gyro_z, imura_buff);
    ...
    //陀螺仪角度转弧度
    source_data.gyro_x = ((float)imu963ra_gyro_x - 5.42) * M_PI / 180 / 14.3f;
    source_data.gyro_y = ((float)imu963ra_gyro_y + 5.39) * M_PI / 180 / 14.3f;
    source_data.gyro_z = ((float)imu963ra_gyro_z - 1.71) * M_PI / 180 / 14.3f;
