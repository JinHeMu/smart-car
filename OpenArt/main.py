...
card_threshold = [((66, 100, -128, 127, -128, 127))]#色块检测阈值
boundary_threshold = [((48, 100, -128, -16, 40, 112))]#边线检测阈值
...
def boundary_correct():
    ...
    while boundary_correct_flag:
        ...
        for l in img.find_line_segments(threshold = 1000, theta_margin = 25, rho_margin = 25):
            if (l.length() > 40 and abs(l.theta()) < 45):
            ...
            elif(l.length() > 40 and abs(l.theta()) > 45) :
                angle = l.theta() # 计算线段的角度
                angle = (angle - 90)
                uart.write("%c" % angle)
                print("Line Angle: ", angle)
...
# card_threshold = [((46, 100, -128, 127, -15, 25))]#色块检测阈值
...
# #         # 创建滑动窗口数组
# #     window_X = [0] * WINDOW_SIZE
# #     window_Y = [0] * WINDOW_SIZE
# #     window_index = 0
...
# #             # 更新滑动窗口数组
# #             window_X[window_index] = dis_X
# #             window_Y[window_index] = dis_Y
# #             window_index = (window_index + 1) % WINDOW_SIZE


# #                         # 计算滤波后的结果
# #             filtered_X = sum(window_X) / WINDOW_SIZE
# #             filtered_Y = sum(window_Y) / WINDOW_SIZE

# #             print("disx:%d, disy:%d, angle:%d" % (filtered_X, filtered_Y, q))

# #             #发送数据
# #             uart.write("C")
# #             uart.write("%c" % int(filtered_X))
# #             uart.write("%c" % int(filtered_Y))
# #             uart.write("%c" % q)
# #             uart.write("D")
# #             lcd.show_image(img, 160, 120, zoom=0)
# #             distance = math.sqrt((dis_X ** 2) + (dis_Y ** 2))
